...
Production deployments benefit from ACE's enterprise features: async operations, intelligent caching, and horizontal scaling.

### Async Retrieval Integration

For high-throughput systems, use async retrieval to avoid blocking:

```python
from ace.async_retrieval import AsyncQdrantBulletIndex
import asyncio

class ACEAsyncEnterpriseAgent:
    """Enterprise agent with async bullet retrieval."""

    def __init__(self, agent, qdrant_url: str = "http://localhost:6333"):
        self.agent = agent
        self.index = AsyncQdrantBulletIndex(qdrant_url=qdrant_url)

    async def run(self, task: str):
        # Async context injection - non-blocking retrieval
        async with self.index:
            relevant_bullets = await self.index.retrieve(task, limit=10)

        # Format bullets for injection
        if relevant_bullets:
            context = "\n".join(
                f"[{r.id}] {r.payload.get('content', '')}"
                for r in relevant_bullets
            )
            task = f"{task}\n\nRelevant strategies:\n{context}"
...
async def main():
    agent = MyAsyncAgent()
    ace_agent = ACEAsyncEnterpriseAgent(agent)
    result = await ace_agent.run("Process data")

asyncio.run(main())
```

**Key Features:**
- `AsyncQdrantBulletIndex` uses `httpx.AsyncClient` for non-blocking I/O
- `batch_get_embeddings()` processes multiple queries in parallel via `asyncio.gather`
- Context manager (`async with`) ensures proper resource cleanup

### Caching Integration

Reduce latency and API costs with intelligent caching:

```python
from ace.retrieval_caching import EmbeddingCache, QueryResultCache
from ace.qdrant_retrieval import QdrantBulletIndex

class ACECachedAgent:
    """Agent with embedding and query result caching."""

    def __init__(self, agent, cache_ttl: int = 3600):
        self.agent = agent
        self.index = QdrantBulletIndex()

        # Embedding cache - stores vector representations
        self.emb_cache = EmbeddingCache(
            max_size=10000,      # Max cached embeddings
            ttl_seconds=cache_ttl  # 1 hour TTL
        )

        # Query result cache - stores search results
        self.query_cache = QueryResultCache(
            max_size=1000,
            ttl_seconds=600      # 10 minute TTL (shorter for fresher results)
        )
    ...
    def run(self, task: str):
        ...
        if cached_results is not None:
            ...
            relevant_bullets = cached_results
        else:
            # Cache miss - retrieve and cache
            relevant_bullets = self.index.retrieve(task, limit=10)
            self.query_cache.set(cache_key, relevant_bullets)

        # Inject and execute
        if relevant_bullets:
            context = "\n".join(b.content for b in relevant_bullets)
            task = f"{task}\n\nStrategies:\n{context}"

        return self.agent.execute(task)

    def _make_cache_key(self, task: str) -> str:
    ...
    def get_cache_stats(self) -> dict:
        ...
        return {
            "embedding_cache": {
                "size": len(self.emb_cache),
                "hit_rate": self.emb_cache.hit_rate
            },
            "query_cache": {
...
# Usage
agent = MyAgent()
ace_agent = ACECachedAgent(agent, cache_ttl=3600)

for task in tasks:
    result = ace_agent.run(task)

# Check cache effectiveness
print(ace_agent.get_cache_stats())
# {"embedding_cache": {"size": 500, "hit_rate": 0.73}, "query_cache": {...}}
```

**Cache Configuration Guidelines:**

| Cache Type | `max_size` | `ttl_seconds` | Use Case |
|------------|------------|---------------|----------|
| EmbeddingCache | 10,000+ | 3600-86400 | High volume, stable embeddings |
| QueryResultCache | 1,000 | 300-900 | Frequently repeated queries |

### Horizontal Scaling Integration

For multi-tenant deployments, use sharded collections:

```python
from ace.scaling import ShardedBulletIndex, QdrantCluster, ShardStrategy, LoadBalancingStrategy

class ACEMultiTenantAgent:
    """Multi-tenant agent with sharded bullet storage."""

    def __init__(self, qdrant_nodes: list, shard_strategy: str = "tenant"):
        # Cluster with load balancing
        self.cluster = QdrantCluster(
            nodes=qdrant_nodes,
            strategy=LoadBalancingStrategy.LEAST_CONNECTIONS
        )

        # Sharded index for tenant isolation
        self.index = ShardedBulletIndex(
            qdrant_client=self.cluster,
            shard_strategy=ShardStrategy.TENANT
        )

    def run(self, task: str, tenant_id: str):
        """Execute task for specific tenant."""
        # Retrieve from tenant's shard only
        relevant_bullets = self.index.retrieve(
            task,
            tenant_id=tenant_id,
            limit=10
        )
    ...
    def learn(self, task: str, result: dict, tenant_id: str):
        """Store learned bullet in tenant's shard."""
        bullet = {
            "content": f"For task '{task[:50]}...': {result.get('lesson', '')}",
            "helpful_count": 1,
            "harmful_count": 0
        }
        self.index.index_bullet(bullet, tenant_id=tenant_id)
...
result = ace_agent.run("Process invoice", tenant_id="acme_corp")
ace_agent.learn("Process invoice", result, tenant_id="acme_corp")

# Monitor cluster
print(ace_agent.get_cluster_health())
```

**Sharding Strategies:**

| Strategy | Collection Pattern | Use Case |
|----------|-------------------|----------|
| `TENANT` | `{tenant_id}_bullets` | Multi-tenant SaaS |
| `DOMAIN` | `{domain}_bullets` | Knowledge specialization |
| `HYBRID` | `{tenant}_{domain}_bullets` | Enterprise with domain separation |

**Load Balancing Strategies:**

| Strategy | Behavior | Best For |
|----------|----------|----------|
| `ROUND_ROBIN` | Cycle through nodes | Uniform load |
| `LEAST_CONNECTIONS` | Route to least busy | Variable query complexity |
| `WEIGHTED` | Proportional to node capacity | Heterogeneous hardware |

### Combined Enterprise Pattern

Production deployments typically combine all three:

```python
from ace.async_retrieval import AsyncQdrantBulletIndex
from ace.retrieval_caching import EmbeddingCache, QueryResultCache
from ace.scaling import ShardedBulletIndex, QdrantCluster

class ACEEnterpriseAgent:
    """Full enterprise integration: async + cached + sharded."""
    ...
    def __init__(
        ...
        # Caching layer
        self.emb_cache = EmbeddingCache(max_size=10000, ttl_seconds=cache_ttl)
        self.query_cache = QueryResultCache(max_size=1000, ttl_seconds=600)

    async def run(self, task: str, tenant_id: str):
        """Full enterprise execution flow."""
        # 1. Check cache
        ...
        if cached:
            ...
            # 2. Async retrieval from correct shard
            bullets = await self._async_retrieve(task, tenant_id)
            self.query_cache.set(cache_key, bullets)

        # 3. Inject and execute
        if bullets:
            context = "\n".join(b.content for b in bullets)
            task = f"{task}\n\nStrategies:\n{context}"

        result = await self.agent.execute(task)

        # 4. Learn to tenant's shard
        await self._async_learn(task, result, tenant_id)
...
Questions? Open a [GitHub issue](https://github.com/kayba-ai/agentic-context-engine/issues)
