...
class TestWrapPlaybookContext:
    ...
    def test_single_bullet_formats_correctly(self):
        ...
        # Should contain usage instructions (TOON format)
        assert "**How to use:**" in result
        assert "h>x (helpful beats harmful)" in result

        # Should contain important note
        assert "Patterns, not rigid rules" in result
    ...
    def test_output_includes_key_sections(self):
        ...
        # Check for TOON format usage instructions
        assert "**How to use:**" in result
        assert "**Note:**" in result

        # Check for specific guidance (TOON format)
        assert "Cite strategy ID when applying" in result
        assert "h>x (helpful beats harmful)" in result
        assert "Match context, adapt to specifics" in result
        assert "Learn from patterns and failures" in result
