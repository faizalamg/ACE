...
def real_llm_client():
    """
    Provides a REAL LLM client for testing.

    Tests using this fixture will be skipped if no API key is available.
    Uses LiteLLM which supports multiple providers.
    """
    if not LLM_AVAILABLE:
        pytest.skip("No LLM API key available (ZAI_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY)")

    from ace.llm_providers.litellm_client import LiteLLMClient
    from ace.config import get_llm_config
    config = get_llm_config()
    return LiteLLMClient(model=config.model, api_base=config.api_base, api_key=config.api_key)
...
def pytest_configure(config):
    ...
    if not pytest_configure_done:
        ...
        config.addinivalue_line(
            "markers", "requires_llm: marks tests that require a real LLM API"
        )
        config.addinivalue_line(
