...
class LiteLLMClient(LLMClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if config:
            ...
            if model is None:
                model = config.model
            # Also use api_base from config if not provided
            if api_base is None:
        ...
        # Auto-add openai/ prefix for custom endpoints with unknown models
        # LiteLLM requires provider prefix for model routing, but custom
        # OpenAI-compatible endpoints (like Z.ai) don't have known prefixes
        model = self._normalize_model_for_custom_endpoint(model, api_base)
    ...
    def _normalize_model_for_custom_endpoint(
        ...
        This allows config to use simple model names like 'glm-4.6' while
        LiteLLM internally routes to the custom endpoint correctly.

        Args:
            model: Model name (may or may not have provider prefix)
            api_base: Custom API base URL (if set, indicates custom endpoint)

        Returns:
            Normalized model name with provider prefix if needed
        """
        # If no custom endpoint, return as-is (use LiteLLM's default routing)
        if not api_base:
            return model
        ...
        # Check if model matches known providers (LiteLLM can route these)
        known_prefixes = [
            "gpt", "openai",  # OpenAI
            "claude", "anthropic",  # Anthropic
            "gemini", "palm", "google",  # Google
            "cohere", "command",  # Cohere
            "llama", "mistral", "mixtral",  # Meta/Mistral
            "bedrock",  # AWS Bedrock
            "azure",  # Azure
        ]
        ...
        for prefix in known_prefixes:
            if prefix in model_lower:
                # Known model, LiteLLM can route it
                return model

        # Unknown model with custom endpoint - assume OpenAI-compatible
        # Add openai/ prefix so LiteLLM routes to the custom api_base
        logger.debug(
            f"Adding 'openai/' prefix to model '{model}' for custom endpoint: {api_base}"
        )
    ...
    def complete(
        ...
        try:
            ...
            # Z.ai GLM models return content in reasoning_content field, not content
            message = response.choices[0].message
            text = message.content
            if not text and hasattr(message, 'reasoning_content') and message.reasoning_content:
                text = message.reasoning_content
                logger.debug("Using reasoning_content as fallback for empty content (GLM behavior)")
