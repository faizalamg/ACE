...
class AdapterBase:
    ...
    def __init__(
        ...
        *,
        ...
        # Propagate unified_index to curator if enabled
        if unified_index and use_unified_storage:
            self.curator.unified_index = unified_index
            self.curator.store_to_unified = True
        elif not use_unified_storage:
            # Explicitly disable unified storage on curator
            self.curator.store_to_unified = False
    ...
    # ------------------------------------------------------------------ #
    def _track_session_outcome(
        self,
        sample: Sample,
        generator_output: GeneratorOutput,
        environment_result: EnvironmentResult,
    ) -> None:
        """Track session outcome for bullets used in this sample."""
        if not self.session_tracker:
            return

        # Determine session type (from constructor or sample metadata)
        session_type = self.session_type or sample.metadata.get("session_type", "default")

        # Determine outcome based on metrics
        # Check for success/correctness indicators in metrics
        outcome = "failed"  # Default to failed
        if environment_result.metrics:
            for key in ["correct", "success", "accuracy"]:
                if key in environment_result.metrics:
                    metric_value = environment_result.metrics[key]
                    # Treat metric > 0.5 as success
                    if isinstance(metric_value, (int, float, bool)):
                        if float(metric_value) > 0.5:
                            outcome = "worked"
                            break

        # Track outcome for each bullet used
        for bullet_id in generator_output.bullet_ids:
            self.session_tracker.track(session_type, bullet_id, outcome)
    ...
    def _process_sample(
        ...
        *,
        ...
        # Track session outcome if session tracker is enabled
        self._track_session_outcome(sample, generator_output, env_result)
