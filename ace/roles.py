...
class Curator:
    ...
    def __init__(
        ...
        *,
        ...
        self.store_to_unified = store_to_unified
        ...
        **kwargs: Any,
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                # Store ADD operations to unified index if enabled
                if self.unified_index and self.store_to_unified:
                    self._store_to_unified(delta)
    ...
    def _store_to_unified(self, delta: DeltaBatch) -> None:
        """Store ADD operations to unified memory index.

        Only ADD operations are stored - UPDATE/TAG/REMOVE are playbook-only.
        Failures are logged but do not interrupt the curation process.

        Args:
            delta: The delta batch containing operations to potentially store
        """
        ...
        try:
            ...
            from .unified_memory import UnifiedBullet, UnifiedNamespace, UnifiedSource

            for op in delta.operations:
                # Only store ADD operations
                if op.type.upper() != "ADD":
                    continue

                # Create unified bullet from operation
                import hashlib
                bullet_id = hashlib.sha256(op.content.encode()).hexdigest()[:12]
                ...
                bullet = UnifiedBullet(
                    ...
                    content=op.content,
                    section=op.section or "task_guidance",
                    helpful_count=0,
                    harmful_count=0,
                )

                self.unified_index.index_bullet(bullet)

        except Exception as e:
            # Log but don't fail - unified storage is supplementary
            import logging
            logging.getLogger(__name__).debug(
                f"Failed to store to unified index: {e}"
            )
