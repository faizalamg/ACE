...
**Field Key (Compressed TOON Format):**
i=id, s=section, c=content, h=helpful, x=harmful, n=neutral(omit if 0)
cf=confidence, tt=task_types, dm=domains, tp=triggers, ap=anti_patterns, cl=complexity

**How to use:**
- Cite strategy ID when applying (e.g., "Following [i: xyz-001]...")
- Prioritize h>x (helpful beats harmful)
- Match context, adapt to specifics
- Learn from patterns and failures

**Note:** Patterns, not rigid rules. Use judgment.\
"""
...
# ================================
# CURATOR ENRICHMENT PROMPT
# ================================

CURATOR_ENRICHMENT_PROMPT = """\
# Semantic Scaffolding Enrichment for Strategy Bullet

You are enriching a strategy bullet with metadata for intelligent retrieval.
This metadata enables purpose-aware retrieval - matching the right strategy
to the right task based on semantic dimensions, not just text similarity.

## Bullet Content to Enrich
{content}

## Usage Context (where this was successful)
{context}

## ENRICHMENT TASK

Extract semantic scaffolding metadata that answers:
1. **WHEN** does this strategy apply? (task types, domains, complexity)
2. **HOW** should it be applied? (preconditions, triggers, anti-patterns)
3. **HOW** should it be found? (retrieval type, embedding text)
...
{{
  "task_types": ["<task category>", ...],
  "domains": ["<domain>", ...],
  "complexity_level": "simple|medium|complex",
  "preconditions": ["<condition that must be true>", ...],
  "trigger_patterns": ["<query patterns that suggest this>", ...],
  "anti_patterns": ["<when NOT to use this>", ...],
  "retrieval_type": "semantic|keyword|hybrid",
  "embedding_text": "<optimized text for vector search>"
}}

## FIELD GUIDELINES

### task_types (REQUIRED)
Categories of tasks this applies to:
- "reasoning" - logical deduction, analysis
- "debugging" - error diagnosis, troubleshooting
- "code_review" - code quality, patterns
- "problem_solving" - multi-step solutions
...
### domains (REQUIRED)
Subject areas this is relevant for:
- Programming: "python", "javascript", "java", "rust", "go", etc.
- Topics: "math", "data_science", "web_dev", "devops", "security"
- General: "general", "communication", "documentation"

### complexity_level (REQUIRED)
- "simple" - Quick, straightforward application
- "medium" - Requires some judgment
- "complex" - Multi-step, requires expertise

### preconditions (RECOMMENDED)
Conditions that should be true for this to apply:
- "error message present"
- "working with numerical data"
- "async operation needed"

### trigger_patterns (RECOMMENDED)
Query patterns that suggest this strategy:
- Key phrases users might use
- Error messages that indicate need
- Situation descriptions

### anti_patterns (RECOMMENDED)
When NOT to use this strategy:
- Situations where it fails
- Edge cases to avoid
- Conflicts with other approaches

### retrieval_type (REQUIRED)
How this should be found:
- "semantic" - Conceptual similarity (most strategies)
- "keyword" - Exact pattern matching (error-specific)
- "hybrid" - Both approaches needed

### embedding_text (REQUIRED)
Optimized text for vector search:
...
Input bullet: "Check for null before accessing object properties"
Context: "Fixed NullPointerException in user service"

Output:
{{
  "task_types": ["debugging", "code_review", "validation"],
  "domains": ["java", "python", "javascript"],
  "complexity_level": "simple",
  "preconditions": ["accessing object property", "object may be null"],
  "trigger_patterns": ["NullPointerException", "TypeError: Cannot read", "AttributeError: NoneType"],
  "anti_patterns": ["primitive types", "already null-checked", "optional types"],
  "retrieval_type": "keyword",
  "embedding_text": "null check validation object property access NullPointerException TypeError AttributeError defensive programming null safety guard clause"
}}
