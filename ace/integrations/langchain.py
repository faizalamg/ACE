...
class ACELangChain:
    ...
    def __init__(
        ...
        =============================
        The ACE learning components (Reflector/Curator) use Z.ai GLM-4.6 by default.
        This requires the ZAI_API_KEY environment variable.

        Your LangChain runnable uses its OWN LLM configuration - ACE does not
        interfere with it. ACE only uses ace_model for learning from results.

        Args:
            runnable: LangChain Runnable (chain, agent, custom)
            ace_model: Model for ACE learning (default: openai/glm-4.6)
                       Common options:
                       - "openai/glm-4.6" (Z.ai, requires ZAI_API_KEY)
                       - "gpt-4o-mini" (OpenAI, requires OPENAI_API_KEY)
            ace_api_key: API key for ACE LLM (optional, uses ZAI_API_KEY env var)
            ace_api_base: API base URL for ACE LLM (optional, auto-configured)
        ...
        Example:
            # Basic (uses Z.ai GLM by default)
            ace_chain = ACELangChain(my_chain)

            # With OpenAI for ACE learning
            ace_chain = ACELangChain(my_chain, ace_model="gpt-4o-mini")
        ...
        import os
        if ace_model == "openai/glm-4.6" or ace_model.startswith("glm-"):
            ace_api_key = ace_api_key or os.getenv("ZAI_API_KEY")
            ace_api_base = ace_api_base or "https://api.z.ai/api/coding/paas/v4"
            if not ace_api_key:
                raise ValueError(
                    "Z.ai GLM requires ZAI_API_KEY environment variable.\n"
                    "Set it in your .env file or pass ace_api_key parameter.\n"
                    "Alternatively, use ace_model='gpt-4o-mini' with OPENAI_API_KEY."
                )
