...
class ACEAgent:
    ...
    def __init__(
        ...
        **agent_kwargs,
        ...
        The browser execution LLM (llm parameter) is SEPARATE and can be any
        LangChain-compatible chat model (ChatOpenAI, ChatAnthropic, etc.).

        Args:
            task: Browser automation task (can also be set in run())
            llm: LLM for browser-use execution (ChatOpenAI, ChatBrowserUse, etc.)
                 This is SEPARATE from the ACE learning LLM.
            browser: Browser instance (optional, created automatically if None)
            ace_model: Model name for ACE learning (default: openai/glm-4.6)
                       Common options:
            ...
            ace_api_base: API base URL for ACE LLM (optional, auto-configured for Z.ai)
